...
6. **정합(두 스캔 합치기) 스모크 - 011·012를 적용했을 때만 해당.** 011·012를 적용하지
   ...
   **(2) 실제 정합을 한 번 돌려 DB 계약을 확인한다.** 같은 바닥면을 겹치게 찍은 스캔
   ...
   > 문구가 바뀌면 이 절차도 함께 고친다 - 이 문서에서 여섯 번 난 Critical이 전부
...
7. **구배 분석의 PDF 보고서 스모크 - 단계 H가 추가한 경로다.** 위 5번(구배 분석
   ...
   > **구배는 전용 장이 아니라 기존 5부 구성에 나뉘어 들어간다.** 새 장 번호를
   > 찾지 마라 - **§2 분석 개요**(구배 개요표·판정 요약·등급 분포·경고),
   > **§3.3 바닥 구배**(셀별 표), **§4 시각자료**(판정 지도)다. 평활도 보고서의
   > 장 번호는 그대로이므로 이 스모크가 기존 절차를 무효화하지 않는다.
   ...
   6. **PDF 본문에서 구배 절 세 곳을 확인한다.** 새 장 번호가 아니라 기존 5부
      구성에 나뉘어 들어간다.
      - **§2 분석 개요**에 `N) 바닥 구배 분석` 머리글. 그 아래 개요표(입력 파일·
        점 수·단위 배율·데이터 계보·분석 파라미터·**판정 가능 비율**·**배수구 위치**·
        엔진 버전·적용 기준)와, **"판정 요약"** 표에 과업지시서 산출 항목이 그대로
        나온다: 열 이름이 **`설계 구배(%)` · `평균편차(%)` · `표준편차(%)` ·
        `최대편차(%)` · `조치 대상 셀(표 게재/전체)`** 이어야 한다. 이어서
        **"등급 분포"** 표에 다섯 등급(`적합`·`경계`·`보수`·`재시공`·`판정불가`)이
        **전수로** 나온다
      - **§3 구간별 결과**에 **`3.3 바닥 구배`** 절. 셀별 표의 열은
        **`위치(cx, cy)` · `등급` · `구배값(%)` · `설계기준 대비 편차(%)` ·
        `방향 편차` · `보정 조치` · `사유`** 다. **역구배 셀은 등급 칸에 `(역구배)`가
        붙고 보정 조치 칸이 `역구배 - 방향 전면 재시공 필요(크기 보정으로 해결 안 됨)`
        여야 한다** - 여기에 `...mm 높임` 같은 크기 문구가 나오면 결함이다(스펙 §7.2.
        크기 편차가 작으면 "고칠 것 없음"으로 읽힌다)
      - **§4 시각자료**에 `N) 바닥 구배 · {측정일자}` 머리글과 **구배 판정 지도
        이미지**, 그 아래 `구배 판정 지도(2.0m 격자 셀별 등급과 내리막 방향).
        배수구 위치: (x, y)` 캡션
      - **지도 이미지 안의 한글**(범례·축)도 함께 본다. 이건 Chromium이 아니라
        **엔진 matplotlib이 만든 PNG**라 보고서 HTML의 폰트 설정과 경로가 다르다.
        네모 상자면 §4의 3번에서 말한 `engine/flatness/outputs/heatmap.py`의
        `matplotlib.rc("font", family=...)` 폴백 체인을 확인한다
      - 조치 필요 셀이 없으면 `3.3 바닥 구배` 아래에 표 대신
        **"조치가 필요한 셀이 없습니다(전체 N셀이 적합 또는 경계)."** 가 나온다.
        정상이다(다음 7번의 `조치필요셀수` 설명 참고)
      - **평활도만 담긴 보고서에는 `3.3 바닥 구배` 절이 아예 없어야 한다.** 머리글만
        있는 빈 절이 보이면 조건 분기가 깨진 것이다
   7. **과업지시서 산출 항목 5개가 발행본 스냅샷에도 박제됐는지 SQL로 확인한다.**
      6번이 화면에서 본 값의 출처이고, 발행 후 재현성은 이 스냅샷이 보증한다.
      SQL Editor에서:
      ...
      - `지도png`는 `reports/{보고서 id}/assets/{분석 id}/slope_map.png` 형태여야
        한다. `null`이면 다음 8번이 실패한 것이고, 그때 §4 시각자료에는 이미지 대신
        "구배 판정 지도(PNG)를 보고서에 담지 못했습니다..." 문구가 나온다
      - **행이 0개면** 스냅샷에 구배 항목이 없다는 뜻이다. 4번에서 구배를 실제로
        체크했는지, 워커가 이 배포의 이미지인지 확인한다
   8. **`slope_map.png`가 발행본 자산으로 복사됐는지 확인한다.** Supabase 대시보드
      **Storage > `reports` 버킷 > `{보고서 id}/assets/{분석 id}/`** 에
      `slope_map.png`가 있어야 한다(7번의 `지도png` 값에서 맨 앞 `reports/`를 뺀
   ...
   > **위 화면·보고서 문구는 저장소의 실제 코드에서 읽어 옮긴 것이다**(후보 목록·차단
   > 사유는 `dashboard/app/reports/new/page.tsx`와 `dashboard/components/report/
   > report-create-form.tsx`, 진행·실패 문구는 `components/report/report-progress.tsx`와
   > `lib/domain/labels.ts`의 `REPORT_GEN_STATUS_LABEL`, 보고서 본문의 구배 절 세 곳은
