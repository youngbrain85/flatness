...
번에 이어 붙이지 말고 두 번 나눠 Run 한다 - 절차와 그 근거는 아래 8·9번 참고).
**011·012도 같은 절차로 두 번 나눠 Run 한다**(아래 11·12번 참고). 나중에
...
8. `supabase/migrations/008_slope_judge_enum.sql` 전체 내용을 붙여넣고 **Run**. `job_type`
   ...
   > **008 다음에 009를 별도로 Run 한다.** 008을 **Run**해서 `Success` 확인 →
   > 에디터를 비우거나 새 쿼리 탭을 연다 → 009를 붙여넣고 다시 **Run**한다.
   >
   > **근거를 정확히 적는다.** PostgreSQL이 같은 트랜잭션 안에서 방금 추가한 enum
   > 값의 *사용*을 "unsafe use of new value"로 막는 것은 사실이고, SQL Editor가
   > 붙여넣은 내용 전체를 한 트랜잭션으로 실행하는 것도 사실이다. 다만 **지금의
   > 009는 그 제약에 실제로 걸리지 않는다** - 실측하면 008+009를 이어붙여 한
   > 트랜잭션으로 실행해도 성공한다(PostgreSQL 16 확인. 함수 3종이 모두
   > `slope_judge` 분기를 가진 채로 커밋된다). 009가 새 값을 **top-level SQL에서
   > 쓰지 않기** 때문이다 - 맨 앞 가드는 `pg_enum.enumlabel`의 **문자열** 비교라
   > enum 값을 쓰는 것이 아니고, 새 값의 실제 사용(`v_job.type = 'slope_judge'`)은
   > 전부 plpgsql 함수 **본문 안**이라 `create function` 시점에는 파싱만 되고
   > 계획이 서지 않는다(9번의 "첫 *실행* 시점에야 계획된다"와 같은 성질이다).
   >
   > **그래도 나눠 Run 하는 것이 정본이다.** (1) enum을 추가하는 마이그레이션은
   > 모두 같은 절차를 따르게 해 두면 파일마다 따져 볼 필요가 없고, (2) 앞으로 009에
   > 함수 본문 **밖**에서 새 값을 쓰는 문장이 하나라도 늘면 합쳐진 파일이 그 순간
   > 막히기 때문이다. 실제로 막히는 형태는 실측으로 확인했다 - 캐스팅
   > (`'slope_judge'::job_type`), `insert ... values ('slope_judge')`, 부분 인덱스
   > 술어, 뷰 정의, 체크 제약, `language sql` + `begin atomic` 본문, 익명 `do`
   > 블록, 그리고 **본문 안에 새 값을 쓴 plpgsql 함수를 같은 트랜잭션에서 호출**하는
   > 것까지 전부 `unsafe use of new value`로 거부된다(HINT: "New enum values must
   > be committed before they can be used"). 즉 지금 통과하는 것은 009가 함수를
   > **만들기만 하고 호출하지 않기** 때문이지 제약이 없어서가 아니다.
...
10. `supabase/migrations/010_scan_height_view.sql` 전체 내용을 붙여넣고 **Run**.
    ...
    **008·009와 달리 나눠 Run 할 일이 없다.** 008·009가 둘로 나뉜 것은 `job_type`
    enum에 새 값(`slope_judge`)을 추가하는 문장과 그 값을 쓰는 코드를 분리해 두려는
    절차상의 선택이다(지금의 009는 008과 한 트랜잭션에 합쳐도 실제로는 통과한다 -
    이유와 실측은 위 8번 참고). 010은 enum을 아예 건드리지 않는 단순
    `alter table ... add column` 하나뿐이라 "unsafe use of new value" 제약과는
    처음부터 무관하다.
...
11. `supabase/migrations/011_register_enums.sql` 전체 내용을 붙여넣고 **Run**.
    ...
    **두 문장이 한 파일에 함께 있는 것은 안전하다** - 서로를 사용하지 않기
    ...
    > 하나라도 늘면 합쳐진 파일이 그 순간 막히기 때문이다. 008·009도 정확히 같은
    > 성질이다 - 실제로 막히는 형태의 실측 목록은 위 8번에 있다.
