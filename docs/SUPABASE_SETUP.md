...
**반드시 001 → 002 → 003 → 004 → 005 → 006 → 007 → 008 → 009 순서로 실행한다** — 뒤
...
있어야 한다(006과는 직접적인 의존 관계가 없다). **008·009는 재판정(구배 배수구
재클릭) 기능에만 필요하다** - 워커·대시보드가 아직 세부과업 4 단계 D를 배포하지
않았다면 지금 당장 건너뛰어도 다른 기능에 영향이 없지만, 나중에 다시 챙기는 수고를
덜려면 지금 함께 실행해 두는 것을 권장한다(단, 008과 009는 **한 번에 이어 실행하면
안 되고 반드시 두 번 나눠 Run 한다** - 아래 8·9번 참고). 이 프로젝트를 워커만 쓰고
대시보드나 보고서 기능을 쓰지 않을 계획이라도 003·004까지 실행해 두는 것을 권장한다
— 나중에 P3/P4를 켤 때 순서를 다시 챙기지 않아도 된다.
...
9. `supabase/migrations/009_slope_judge_functions.sql` 전체 내용을 붙여넣고 **Run**.
   ...
   **`analyses.status`는 이 분기에서 절대 바뀌지 않는다** - 재판정은 이미
   ...
   재생성이 필요했다). 검증: `select proname from pg_proc where proname in
   ('fn_job_claim','fn_job_fail','fn_reap_stuck_jobs');` 3행이 오류 없이 나오면(함수가
   존재하면) 성공 - `create or replace`라 이 쿼리만으로는 slope_judge 분기가 실제로
   반영됐는지까지는 확인할 수 없고, 온전한 기능 검증은 세부과업 4 단계 D의 워커·
   대시보드가 배포된 뒤 배수구 클릭으로 확인한다.
