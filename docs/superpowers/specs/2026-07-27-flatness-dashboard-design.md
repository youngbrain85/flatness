...
1. **로드·검증**
   - 런치 파서: PLY(binary/ascii), LAS/LAZ(laspy), XYZ/TXT/CSV, PTS(ASCII). 청크 스트리밍/메모리맵, float32. E57은 인터페이스만(§2.2)
...
6. **직선자 시뮬레이션 (판정 근거)**
   - **윈도우 기하**: 판정 셀(기본 1m)마다 **셀 중심에 정렬된 span_m 길이 윈도우**, 동일 구역 점만 사용, 4방향(0/45/90/135°) 1D 프로파일 → 상부 볼록 포락선(실물 직선자 지지선) → 포락선 아래 최대 틈새. 윈도우 점유율 70% 미만 또는 공분산 조건수 미달 셀은 '판정 불가'
...
- 원시 점군은 청크 스트리밍으로 읽어 **즉시 float32 서브셀 비닝** — 원시 전체를 Open3D 객체로 만들지 않는다(Open3D는 다운샘플 이후에만 사용)
