...
- 출처: 단계 G(용역 결과 보고서) 착수 전 백로그 정본성 완결성 비평. 단계 G가
  이 파일에서 미이행 항목을 뽑아 대외 제출 문서에 반영하므로, 실제로
  아직 남아 있는 결함만 여기 남긴다
...
81. [해결: 커밋 `3bf3b8f`] **엔진이 만드는 `slope_map.png`에는 배수구 마커가 없다** —
    ...
    **왜 문제인가**: 판정표에서 "이 셀이 왜 역구배(재시공)인가"의 답은
    ...
    받는 쪽은 그 jsonb 값을 볼 방법이 없다 - 그림이 스스로 완결된
    설명이 되지 못한다.
...
## 세부과업 4 단계 F 최종 리뷰 이연 티켓 (2026-08-08)

- 출처: 단계 F(정합 엔진 + 정합 화면 + 병합 스캔, 머지 커밋 `880b283`) 완료 후
  단계 G(스캔 가이드라인 갱신 + 용역 결과 보고서 편입) 착수 전 최종 리뷰가
  이연 판정한 항목

86. **`estimate_normals` 청크화 회귀 가드 부재** - 프로덕션 코드는 옳다. 다만
    청크화를 되돌리는 회귀가 나도 지금 스위트는 잡지 못한다. 잡으려면 청크
    경계를 실제로 넘나드는 표본에서 결과가 달라짐을 단언하는 `perf` 마커
    테스트가 필요한데, 그 규모의 픽스처가 5cm 서브셀 기준 약 1.7M점(약 4,000
    m²)부터 메모리 2GiB를 소진한다. 지금 성능 게이트(3천만 점, 5분, 2GiB)를
    이 테스트 하나가 잠식할 위험이 있어, 게이트 예산을 별도로 확보하거나
    청크 경계만 작게 재현하는 축소 픽스처를 설계하기 전까지 이연한다
87. **`/registrations` 목록 화면 없음** - 실패한 정합에 다시 도달할 경로가
    URL 직접 입력뿐이고, 버려진 `awaiting_points` 행이 쌓여도 화면에서
    이를 정리할 방법이 없다. 단계 F 스코프는 정합 성공 경로(대응점 클릭부터
    병합까지)를 완결하는 것이었고 실패 후 재접근은 기능을 막지 않는
    불편함이라 목록·정리 화면은 별도 작업 단위로 이연했다
88. **`stats.artifacts.map_png` 형식을 단언하는 테스트 0건**(기존 공백) -
    단계 F 이전부터 있던 공백이라 F가 만든 회귀는 아니다. 정합 화면의
    겹쳐보기가 같은 자산 경로 관례를 재사용하며 최종 리뷰가 인접 코드를
    훑다가 이 공백을 다시 발견해 여기 통합 기록한다
89. **`upsert_scan`의 PostgREST 경로 단위 테스트 없음** - 병합 스캔
    멱등성이 `Prefer: resolution=merge-duplicates` 헤더에 달려 있는데
    배포 스모크에서만 확인된다. `SupabaseRest` 클래스 전체가 PostgREST
    요청 헤더·쿼리 파라미터를 단위 테스트하지 않고 배포 스모크로만
    검증하는 관례를 따르고 있어(예: `set_current_analysis`의 2단계 PATCH도
    같은 방식, 티켓 29 참고), 이 함수 하나만 예외로 단위 테스트를 두면
    같은 클래스 안에서 검증 수준이 들쭉날쭉해진다. 예외를 만들 근거가
    부족해, `SupabaseRest` 전체의 PostgREST 경로 테스트 전략을 다시 정하는
    별도 작업으로 이연한다
90. **`CORR_CLICK_SD_M`과 `_TARGET_LONG_SIDE`의 관계가 코드에 없다** -
    클릭 정밀도가 현장 크기에 반비례하는데 두 상수가 코드에서 연결돼
    있지 않아 한쪽만 바꾸면 다른 쪽과 조용히 어긋난다. 두 상수를 잇는
    정확한 함수 형태는 실측 대조(실물 직선자) 데이터 없이는 근거 있게
    정할 수 없어(세부과업 1의 U 재설정 미수행과 같은 사유), 관계식을
    코드화하는 대신 이번에는 기록만 남긴다
91. **`horizontal_sensitivity`가 실패한 정합에서 0.0을 낸다** - `nan`/
    `0.0`/`<1.1`이 서로 다른 뜻(계산 불가/구조적으로 무의미/정상 감도)인데
    필드 하나가 이를 구분하지 못한다. 구분하려면 화면과 `reports.snapshot`이
    이미 소비하는 필드의 타입이나 계약을 바꿔야 해서(예: 실패 사유를 담는
    별도 필드 신설), 로컬 수정이 아니라 하위 소비자까지 걸리는 계약 변경이라
    이연한다
92. **`showFirstFlatness`의 `archived` 경계가 테스트로 안 잡힌다**(`archived`를
    쓰는 코드 경로가 아직 없어 잠재적) - 지금은 `archived`를 실제로 세팅하는
    코드 경로가 없어 경계를 어겨도 회귀가 발생할 방법이 없다. 그 코드 경로가
    생길 때 테스트를 함께 추가하는 편이 지금 잠재 결함에 테스트부터 먼저
    쓰는 것보다 정확하다
