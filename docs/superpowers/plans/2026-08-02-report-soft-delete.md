...
**브랜치 시작점부터 HEAD까지 전 구간을 검사한다.** 최근 커밋 몇 개로 범위를
좁히면(예: `HEAD~3`) 그보다 앞서 이 브랜치에서 커밋된 마이그레이션·도메인 문자열
변경이 검사에서 빠진다. `main`과 갈라진 지점을 `git merge-base`로 구해 그 지점부터
diff한다.

**`dashboard`·`supabase` 코드 경로만 스캔한다.** 이 계획 문서(`docs/`) 자체에는
em-dash 검사 방법을 설명하는 주석·예시 명령이 있고 그 안에 검색 대상 글리프
`—`가 문자 그대로 들어간다 - 문서·주석은 전역 규칙상 허용 대상이라 여기 걸리는
건 실제 위반이 아니다. `docs/`까지 diff에 넣으면 이 자기참조 때문에 `0`이 아닌
값이 나와 검사 자체가 거짓 양성을 낸다.

Run:
```bash
BASE=$(git merge-base main HEAD)
git diff "$BASE" -- dashboard supabase | grep "^+" | grep -c "—"
```
...
**주의:** 마이그레이션을 실행하기 전에 새 대시보드 코드가 배포되면, `.is('deleted_at', null)` 필터가 존재하지 않는 컬럼을 참조해 오류가 난다. 보고서 목록(`app/reports/page.tsx`)과 상세(`app/reports/[id]/page.tsx`)가 같은 필터를 쓰므로 **둘 다** 오류로 뜬다. **마이그레이션을 먼저 실행할 것.**
