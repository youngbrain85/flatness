...
### 구배 분석 항목 (세부과업 4 단계 H 추가, `schema` 버전은 그대로)

구배 분석(`analyses.kind='slope'`)이 보고서에 포함되면 `analyses[]`에 **위 표와 다른 형태의 원소**가 들어간다. 등급 어휘와 통계 스키마가 평활도와 전부 달라 같은 키를 재사용하지 않았다.

> **★ 판별은 반드시 긍정형 `kind == 'slope'`다.** `kind` 키는 **구배 항목에만** 싣는다. 평활도 항목에 `kind: 'flatness'`를 더하면 이미 발행된 스냅샷(발행본은 다시 만들지 않으므로 그 키가 영원히 없다)과 새 보고서가 다른 내용을 담게 된다. 따라서 `kind == 'flatness'`나 `kind != 'slope'`로 평활도를 **긍정 판별**하면 옛 발행본에서 전부 어긋난다. `schema`를 올리지 않은 이유가 이것이다: 평활도 항목은 바이트 단위로 동일하고, 구배 항목은 새 키로 자기 식별한다.

| 키 | 타입 | 내용 |
|---|---|---|
| `kind` / `kind_label` | `"slope"` / string | 구배 항목에만 존재. `ANALYSIS_KIND_LABEL['slope']` = `"구배"` |
| `analysis_id`, `sort_order`, `surface`, `surface_label`, `engine_version`, `is_external`, `scan`, `warnings`, `auto_summary`, `user_summary` | 위 표와 동일 | 식별·스캔 정보는 평활도와 같은 키·같은 뜻. 단 `warnings[].text`는 엔진이 낸 **완성된 한국어 문장**이다(평활도의 슬러그 해석 결과가 아니다) |
| `criteria` | object | `{name, source}` **둘뿐이다.** 구배 기준 행에는 `span_m`·`pass_mm`·`rework_mm`·`u_mm`이 없다 |
| `overview` | object | `{file, n_points, scale_to_m, subcell_m, cell_m, coverage_pct, coverage_label}`. `coverage_label`은 `"판정 가능 비율"`(= 판정된 셀 / 전체 셀). 평활도의 바닥 인식률과 **뜻이 다르다**. `reduced_span_cells`는 없다 |
| `summary` | object | `{overall_verdict, overall_verdict_label}` **둘뿐이다.** mm 통계·등급 분포는 넣지 않는다 - 0이나 null로 채우면 측정한 적 없는 수치가 진짜 결과처럼 인쇄된다 |
| `sections` | 없음 | 구간별 결과는 `slope.cells`가 대신한다 |
| `assets` | object | 평활도 키를 모두 갖되 전부 비어 있고, `slope_map`(`str\|null`)이 추가된다 |
| `slope` | object | 아래 표. 과업지시서 11쪽 산출 항목이 전부 여기서 나온다 |

`slope` 원소:

| 키 | 타입 | 내용 |
|---|---|---|
| `design_pct` | float\|null | **설계 구배(%)**. `stats.threshold`에서 읽는다(재판정이 `applied_criteria`를 갱신하지 않으므로 컬럼이 아니라 stats가 정본이다) |
| `dev_mean_pct` / `dev_sd_pct` / `dev_max_pct` | float\|null | **평균편차 · 표준편차 · 최대편차(%)**. 과업지시서 산출 항목 |
| `pass_pct` / `re_pct` / `dir_pass_deg` | float\|null | 적합 폭 · 재시공 임계 · 방향 허용각(도) |
| `direction_judged` | bool | `false`면 배수구 미지정으로 **방향(역구배) 판정을 하지 않은** 결과다. 보고서가 이 사실을 밝혀야 한다 - 크기만 본 "적합"이 섞여 있다 |
| `cell_m` / `n_cells` | float\|null / int | 판정 격자 한 변(2m)과 **전체** 셀 수 |
| `counts` | object | 다섯 등급 전수 `{적합, 경계, 보수, 재시공, 판정불가}`. `cells`가 조치 대상만 싣기 때문에 **빠진 셀을 설명하는 것이 이 키다** |
| `drain_points` | object[] | `{x, y}` 미터 좌표. 미지정이면 `[]` |
| `map_png` | string\|null | `reports/{report_id}/assets/{analysis_id}/slope_map.png`. 복사 실패 시 `null`이고 사유는 최상위 `notes`에 남는다 |
| `cells` | object[] | **조치가 필요한 셀만**(`보수`·`재시공`·`판정불가`. 역구배는 등급이 재시공이라 포함된다). `(cy, cx)` 오름차순. 2m 격자 수백 개를 전부 실으면 표가 보고서를 채우므로 적합·경계는 뺀다 |
...
|---|---|---|
| `cx` / `cy` | int | 격자 좌표 |
| `grade` / `reason` | string / string | 한국어 등급명과 판정 사유 |
| `slope_pct` / `dev_pct` | float\|null | **구배값(%)** 과 **설계기준 대비 편차(%)**. 과업지시서 산출 항목 |
| `reverse` | bool | 역구배 여부. 스펙 §7.2가 색만으로 구별하는 것을 금지하므로 **문구 배지용**이다 |
| `correction_text` | string | **역구배면 방향 문구**(`"역구배 - 방향 전면 재시공 필요(크기 보정으로 해결 안 됨)"`), 그 외에는 크기 문구, 산출 불가면 `"-"`. 정본은 `dashboard/lib/domain/slope-direction.ts`. **역구배에 크기 문구를 내면 `"서쪽 끝을 0.0mm 높임"` 같은 "고칠 것 없음"으로 읽히는 문구가 나온다** |
| `dir_err_deg` / `direction_text` | float\|null / string | 방향 편차와 그 표기(`"45.0도(허용 30도 초과)"` 또는 `"-"`). 역구배까지는 아니지만 허용각을 넘은 셀을 드러낸다 |

**불변 규칙**: (a) snapshot 내 모든 asset 경로는 `reports/{report_id}/assets/…` 버킷-상대 문자열이다. (b) 렌더러는 snapshot에 없는 값을 절대 계산하지 않는다. (c) 스키마를 바꾸면 `schema` 문자열의 버전을 올리고 본 절을 갱신한다. (d) **평활도 항목에 키를 더하지 않는다** - 옛 발행본과 새 보고서가 갈린다.
