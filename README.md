...
발주처 제공물이 BIM 모델이 아니라 **PDF 도면**이라, 도면에서 기하를 복원해 모델을 만들었다.
...
욕실은 슬래브가 150mm 내려가 있어 **두께(THK)를 레벨로 읽으면 높낮이 방향이 뒤집힌다** —
두 도면(부분상세도 FL/SL ↔ 마감표 THK)이 `FL − SL = THK`로 서로를 검증한다. 판정 결과,
상업용 서빙·배송·청소·산업 AMR **4개 등급 전부 현관 30mm 단차에 막힌다**(임계값은 전부
...
0.0025% 이내. 도면에 없는 값(유효 통과폭·미기재 천장고)은 지어내지 않고 `unknown`·명목값
표식으로 남긴다.

### 과업 3 — 스캔 커버리지 시뮬레이션

로봇의 작업은 LiDAR 스캔이다 — 계획된 경로가 점군 밀도를 결정하고, 그 점군이 과업 1·4의
분석 입력이 된다. 커버리지 기준은 면적이 아니라 **점 밀도**(모바일 ≤20mm / TLS ≤5mm).
...
TLS 거치점 배치는 탐욕 set cover, 순회는 최근접 이웃 + 2-opt. 같은 기하를 SDF world로
변환해 **Gazebo(독립 물리 엔진)에서 같은 경유점을 주행** — 운행 거리 오차 +0.07%.

### 과업 1·4 — 평활도·구배 실측 분석 (배포된 파이프라인)

업로드 → 분석 → 판정 히트맵 → PDF 보고서가 웹 대시보드에서 돈다. 아래는 합성 demo 점군
산출물이다(실물 직선자 실측 대조는 미수행 — [보고서](docs/service-report.md) 6장).
...
이 프로젝트의 회귀 기준은 단언 개수가 아니라 **심은 변이를 몇 개 죽였는가**다 — "프로덕션
코드는 맞는데 테스트가 정작 막으려던 회귀를 못 잡는" 사고가 반복되어, 변이 실험에 **무변이
대조군**을 필수로 두었다(없으면 "전부 차단"이 도구가 조용히 죽은 상태와 구별되지 않는다).

수행하지 않은 검증은 수행했다고 적지 않는다: 실물 직선자 실측 대조, 실물 드론·지상 스캔
대조, 실물 로봇 주행은 **미수행**이며 보고서에 그렇게 명시되어 있다. 예외적으로 도면 복원
정확도만은 합성이 아니라 **발주처가 도면에 인쇄한 면적표와의 대조**다.
...
> 과업지시서 원문과 발주처 도면 PDF는 저장소에 포함하지 않는다(`data/` gitignore).
> 저장소의 도면 관련 데이터는 전부 **파생 기하**(실 폴리곤·레벨·가구 좌표)다.
