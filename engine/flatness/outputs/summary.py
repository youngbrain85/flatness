...
_WARN_TEXT = {
    ...
    # 형제 경고(swallows_repair)가 이미 종합의견에 실리므로 함께 싣는다. 이쪽이 더
    # 중요하다 - 편차가 0이어도 적합이 안 나오므로, 결과를 "벽이 나쁘다"로 오독하지
    # 않도록 종합의견 본문에서 사유를 알려야 한다.
    "uncertainty_swallows_pass": "측정 불확도가 허용치보다 커서 적합 판정이 나올 수 없는 기준입니다. 기준 또는 측정 불확도 재검토가 필요합니다.",
