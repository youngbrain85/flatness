...
def test_render_height_view_png_puts_low_y_bump_at_bottom_of_image(tmp_path, monkeypatch):
    ...
    **이 테스트가 잡지 못하는 것(재리뷰 실측)**: 전치(img.T)는 이 반평면 단언으로
    구별되지 않는다. 정규화 정점이 (0.30, 0.233)이라 전치해도 (0.233, 0.30)으로
    여전히 같은 사분면에 남기 때문이다. 이면체군 8종 중 나머지 6종(좌우·상하·
    180도·반전치·rot90·rot270)은 전부 잡힌다. 전치 회귀는 플레인 PNG 테스트가
    좌표로 잡는다(x 기댓값 1.2와 y 기댓값 0.7이 서로 달라 전치가 값에 드러난다).
    임계값을 조여 0.704 대 0.658을 가르려 들지 마라 - 오히려 취약해진다.
