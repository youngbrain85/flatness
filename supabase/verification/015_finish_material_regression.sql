...
--   4) 스코프는 **도면·코드체계·기준세트**로 못박는다. 두 번째 발주처 도면이
...
-- -----------------------------------------------------------------------------
-- 스코프: 이 도면 / 이 발주처 코드체계 / 이 기준세트 만 본다.
-- -----------------------------------------------------------------------------
...
-- =============================================================================
\echo ''
\echo '=== A01. 스코프가 값으로 확정된다 (도면 1 / 코드체계 1 / 기준세트 1) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A02. 실 9행: 이름·면적(4자리)·FL·SL·근거 가 GROUND_TRUTH 와 집합 동등 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A03. 근거 축과 면적 축의 관계: drawing_confirmed ⇔ 면적 보유, inferred 는 사유 필수 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A04. 레벨 원문 모순 2건이 양쪽 다 살아 있다 (주기13 ↔ 그래픽 라벨) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A06. 인접 8행: 쌍·종류·단차(부호)·절대값·낮은쪽·근거 가 집합 동등 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A07. 쌍 정규화 결정성: 어느 방향으로 넣어도 같은 행으로 수렴한다 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A08. v_space_step 파생축: 원문모순 전파 · 추론실 접촉 · 근거 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A09. ★임계값 29행 전량을 값으로 검사한다 (관대하게 되돌리면 여기서 죽는다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A10. 임계값 근거(evidence) 무결성: 비우면 죽는다 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A11. ★형상 조건부 임계값: 수직 문턱 10mm 는 fail, 모따기 10mm 만 marginal ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A12. ★조합 규칙 그룹: boolean 멤버 혼합 + 형상 게이트 (전부 되돌린다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A13. 상업용 청소로봇 단차 판정 8행 × 2모드 (임계값을 관대하게 하면 죽는다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A14. ★유효 통과폭은 어떤 개구부도 확정 불가 — 제작치수를 대입하면 죽는다 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A15. ★material_properties 8행: 규격 요구값이 실측 물성 자리에 들어오면 죽는다 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A16. 관측값 단위 가드: inch 를 mm 물성 자리에 넣으면 23514 로 막힌다 ==='
-- =============================================================================
do $$
declare
...
-- =============================================================================
\echo ''
\echo '=== A17. subject=space 관측 저장소가 실재한다 (6지표 수용 · 타 subject 거부) ==='
-- =============================================================================
do $$
declare
  v_sp uuid; r record;
...
-- =============================================================================
\echo ''
\echo '=== A18. 침묵하면 추론분이다: basis 기본값 inferred + 사유 없는 추론은 CHECK 로 막힌다 ==='
-- =============================================================================
do $$
declare
...
-- =============================================================================
\echo ''
\echo '=== A19. space_finishes 48행 전량 (절반을 지우면 죽는다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A20. project_codes 36행 전량 (창호·단열·방수·코킹 탈출구 포함) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A21. project_code_layers 34행 전량 (전량 삭제하면 죽는다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A22. 마감재 계열 트리 14행 + 제품군→계열 배정 52행 (뒤섞으면 죽는다) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A23. v_finish_taxonomy 조립 무결성: 부위 > 계열경로 > 제품군 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A24. "26형 각 실의 바닥 마감재는?" — 4조인 질의 결과가 실별로 하나씩 ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A25. ★fn_eval_threshold 진리표: lte/lt/gte/gt/eq/neq 전 분기 (G1) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A26. ★구조 가드의 존재 자체 (제약·트리거) — 지우면 여기서 죽는다 (G2) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A27. ★구조 가드의 동작: 위반 삽입이 실제로 거부되는가 (G2·F5·F6·F7) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A29. 행 수 고정 — EXCEPT 대칭차의 방향성 구멍을 막는다 (G4) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A30. 인접의 판정 입력 컬럼 전량: profile · 상한 · 틈 · 창호코드 (G5·F5) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A31. v_space_step 파생 표식 step_is_derived · step_unknown (G9·F3) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A32. ★등급 × 모드 전조합에 규칙이 0건인 조합이 없다 + 폴백 모드 대조 (F2) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A33. ★등급 상속: 자식이 부모를 이긴다 (형상 특정성보다 깊이가 먼저) (F4) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A34. 무검증이던 제원 축: 천장고 · 지표 측정가능성 · 등급 제원 · 경도 · 신뢰도 (G7) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A35. 시험조건(test_cond)과 근거(evidence) — 조건이 값의 의미를 결정한다 (G6) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A36. RLS 정책 50개 전량 (Supabase 공개 PostgREST 대상이다) (G8) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A37. 서술 필드가 값과 모순되지 않는다 (G10) ==='
-- =============================================================================
...
-- =============================================================================
\echo ''
\echo '=== A38. outline 폴리곤 ↔ area_m2 대조 (셰이스 공식) ==='
-- =============================================================================
-- outline 은 도면 벡터에서 복원한 기하이고 area_m2 는 발주처가 인쇄한 면적표다.
...
begin
  ...
  if v_fail > 0 or v_missing is not null then
    raise exception '★회귀 실패: PASS %건 / FAIL %건 / 누락 [%]',
      v_pass, v_fail, coalesce(v_missing, '없음') using errcode = 'P0001';
  end if;
